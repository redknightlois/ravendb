...
namespace Micro.Benchmark.Benchmarks.Tensors
{
    ...
    public class CosineSimilarityBench
    {
        ...
        [Benchmark]
        public float FloatSerial()
        {
            return Functions.Serial.CosineSimilarity<float, float>(_floatValuesA, _floatValuesB);
        }

        [Benchmark]
        public double DoubleSerial()
        {
            return Functions.Serial.CosineSimilarity<double, double>(_doubleValuesA, _doubleValuesB);
        }

        [Benchmark]
        public float FloatVectorized_SingleLane()
        {
            return Functions.Vectorized256.CosineSimilarity<float, float>(_floatValuesA, _floatValuesB);
        }

        [Benchmark]
        public float FloatVectorized_DoubleLane()
        {
            return Functions.Vectorized256.CosineSimilarity512<float, float>(_floatValuesA, _floatValuesB);
        }

        [Benchmark]
        public float FloatVectorized_DoubleLane_WithBackLoad()
        {
            return Functions.Vectorized512.CosineSimilarity<float, float>(_floatValuesA, _floatValuesB);
        }

        [Benchmark]
        public double DoubleVectorized()
        {
            return Functions.Vectorized256.CosineSimilarity512<double, double>(_doubleValuesA, _doubleValuesB);
        }
    }
}
