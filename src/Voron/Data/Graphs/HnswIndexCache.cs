...
/// point, promoted-only) and incrementally maintained by <see cref="ApplyCommit"/> after
/// each indexing commit. Capacity is provisioned at construction time as a count of
/// promoted nodes; if the buffer fills, further <see cref="TryAppend"/> calls fail
/// gracefully and those nodes simply miss the cache.
/// </summary>
public sealed unsafe class HnswIndexCache : IDisposable
{
    ...
    /// <summary>
    /// Initial fill: BFS from the entry point through all upper levels (>= 1), admitting every
    /// node it visits whose <see cref="Hnsw.Node.EdgesPerLevel"/> count is at least 2 (i.e. the
    /// node participates at level 1 or higher and so functions as a router during search).
    /// Level 0 is never expanded, so leaves never enter the cache. BFS allocation order means
    /// neighboring promoted nodes also become neighboring records in <see cref="_buffer"/>,
    ...
    public static HnswIndexCache WarmFromScratch(LowLevelTransaction llt, Slice fieldName, int maxNodes)
    {
        ...
        for (int level = maxLevel; level >= 1 && builder.HasBudget; level--)
    ...
    /// (id collision), so re-touched nodes keep their previous cache entry. Promoted-only
    /// policy is enforced.
    /// </summary>
    ...
    private sealed class WarmupBuilder : IDisposable
    {
        ...
        /// <summary>
        /// Bulk-emit every promoted node into <paramref name="cache"/>. Promoted means
        /// <see cref="Hnsw.Node.EdgesPerLevel"/> count >= 2 (the node lives at level 1 or
        /// higher and is therefore a router for upper-level traversals).
        /// </summary>
