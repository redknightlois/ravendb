...
namespace Sparrow.Server.Tensors
{
    public static class Arithmetics
    {
        ...
        public static T ReciprocalSqrtEstimate<T>(T value)
            where T : unmanaged, IFloatingPoint<T>, IRootFunctions<T>, INumber<T>
        {
            if (AdvInstructionSet.Arm.IsSupported)
            {
                if (typeof(T) == typeof(float))
                {
                    return (T)(object)(float)Math.ReciprocalSqrtEstimate((float)(object)value);
                }
                else if (typeof(T) == typeof(double))
                {
                    return (T)(object)Math.ReciprocalSqrtEstimate((double)(object)value);
                }
            }

            // TODO: Implement the version for Intel and AMD architectures using Ralph

            return T.One / T.Sqrt(value);
        }
    }
}
