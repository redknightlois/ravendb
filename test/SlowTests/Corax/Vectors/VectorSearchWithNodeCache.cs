...
public class VectorSearchWithNodeCache(ITestOutputHelper output) : StorageTest(output)
{
    ...
    // haven't committed yet so no cache exists.
    [RavenFact(RavenTestCategory.Vector | RavenTestCategory.Corax)]
    public void SearchWorksWithNoCacheAttached()
    {
        using var _ = GetMappings(VectorEmbeddingType.Single, out var bsc, out var mapping);
        var metadata = mapping.GetByFieldId(1).Metadata;
        BuildIndex(mapping, docCount: 200, seed: 17);

        using var searcher = new IndexSearcher(Env, mapping);
        // Deliberately do NOT call AttachVectorNodeCaches.
        var results = TopK(searcher, metadata, NewQueryVector(bsc, seed: 71), k: 16);
    ...
    [RavenFact(RavenTestCategory.Vector | RavenTestCategory.Corax)]
    public void SearchWorksWithEmptyCacheDictionaryAttached()
    {
        using var _ = GetMappings(VectorEmbeddingType.Single, out var bsc, out var mapping);
        var metadata = mapping.GetByFieldId(1).Metadata;
        BuildIndex(mapping, docCount: 200, seed: 19);

        using var searcher = new IndexSearcher(Env, mapping);
        searcher.AttachVectorNodeCaches(new Dictionary<Slice, Hnsw.NodeCache>(SliceComparer.Instance));
        var results = TopK(searcher, metadata, NewQueryVector(bsc, seed: 73), k: 16);
        Assert.Equal(16, results.Length);
    }
    ...
    // the NodeCache it was given was built AT OR BEFORE that snapshot. A cache that predates
    ...
    private Dictionary<Slice, Hnsw.NodeCache> BuildCaches(Slice fieldName, int maxNodes = 10_000)
    {
        using var tx = Env.ReadTransaction();
        var cache = Hnsw.NodeCache.Build(tx.LowLevelTransaction, fieldName, maxNodes);
