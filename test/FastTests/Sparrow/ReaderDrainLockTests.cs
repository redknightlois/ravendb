...
namespace FastTests.Sparrow
{
    public class ReaderDrainLockTests : NoDisposalNeeded
    {
        ...
        public void TryEnterRead_FailsWhileWriterPending()
        {
            ...
            writerPending.Wait();
            SpinWait.SpinUntil(() => l.TryEnterRead(out ReaderDrainLock.ReadHandle h) == false || ProbeAndRelease(l), TimeSpan.FromSeconds(1));
        ...
        private static bool ProbeAndRelease(ReaderDrainLock l)
        {
            if (l.TryEnterRead(out ReaderDrainLock.ReadHandle h))
            {
                h.Dispose();
                return true;
            }
