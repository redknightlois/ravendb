...
public unsafe class HnswIndexCacheTests(ITestOutputHelper output) : StorageTest(output)
{
    ...
    // Promoted-only policy: only nodes that participate at level >= 1 are admitted. With a small
    // graph the BFS will catch every promoted node when the budget is generous. Leaves
    // (LevelCount == 1) are not present.
    [RavenFact(RavenTestCategory.Voron | RavenTestCategory.Vector)]
    public void BudgetLargerThanGraphCachesAllPromotedNodes()
    {
        using var _ = Slice.From(Allocator, TreeName, out var treeName);
        ...
        using (var tx = Env.ReadTransaction())
        {
            ...
            Assert.True(cache.Count > 0, "cache should have at least the promoted set");
            Assert.True(cache.Count <= vectorCount, "cache count should not exceed graph size");

            // No cached node may be a leaf; every record must report LevelCount >= 2.
            foreach (var (nodeId, view) in EnumerateCache(cache, vectorCount))
            {
                Assert.True(view.LevelCount >= 2,
                    $"cached node {nodeId} has LevelCount {view.LevelCount} but only promoted nodes (>=2) are admitted");
            }
        }
    }
