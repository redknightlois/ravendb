...
public class L2NormCaching(ITestOutputHelper output) : StorageTest(output)
{
    ...
    public void ZeroNormAdversarial()
    {
        ...
        // does not prescribe more than "not NaN" for this case, but 1f is what the kernel
    ...
    {
        // Construct an Options struct pinned to the old on-disk layout and verify the kernel
        // dispatcher still returns the legacy norm-recomputing path. This locks the version
        // gate in place so an accidental bump won't silently break older indexes.
