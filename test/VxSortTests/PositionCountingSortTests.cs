...
namespace Test
{
    public class PositionCountingSortTests
    {
        ...
        static IEnumerable<TestCaseData> PreSorted =>
            ...
            ).SetArgDisplayNames($"S{size}");

        static IEnumerable<TestCaseData> HalfMinValue =>
            from size in _pcSortSizes
            from seed in new[] { 666, 333, 999, 314159 }
            select new SortTestCaseData(
                () => GenerateData(size, seed, int.MinValue, 0.5)
            ).SetArgDisplayNames($"{size}/{seed}/0.5min");
        ...
        static IEnumerable<TestCaseData> HalfMaxValue =>
            ...
            ).SetArgDisplayNames($"{size}/{seed}/0.5max");

        static IEnumerable<TestCaseData> ConstantSeed =>
            from size in _pcSortSizes
            from seed in new[] { 666, 333, 999, 314159 }
            select new SortTestCaseData(() => GenerateData(size, seed)).SetArgDisplayNames(
                $"{size}/{seed}"
            );
        ...
        static IEnumerable<TestCaseData> AllTests16 =>
            PreSorted16
                .Concat(HalfMinValue16)
                .Concat(HalfMaxValue16)
                .Concat(ConstantSeed16)
                .Concat(TimeSeed16);
    }
}
